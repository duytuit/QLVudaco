...
{
    public partial class ucBangTheoDoiDebit_KoFile_NCC : DevExpress.XtraEditors.XtraUserControl
    {
        ServiceReference1.WebService1SoapClient client = new ServiceReference1.WebService1SoapClient();
        public ucBangTheoDoiDebit_KoFile_NCC()
        {
            InitializeComponent();
        ...
        private void btnXem_Click(object sender, EventArgs e)
        {
            ...
            if (arr1.Length >= 3 && arr2.Length >= 3 && arr1[0].Trim() != "" && arr2[0].Trim() != "")
            {
                ...
                {
                    gridControl1.DataSource = _ncc.DieuXeKhongCoFile_NCC(Ngay1, Ngay2);
                    gridControl2.DataSource = _ncc.BangFileDebitNCCKhongLapFile(Ngay1, Ngay2);
                }
            }
        }
        ...
        private void repositoryItemSua_Click(object sender, EventArgs e)
        {
            ...
            frmFileDebitNcc frm = new frmFileDebitNcc(0, 0, 1, 0, 0, _madieuxe);
            frm.ShowDialog();
            btnXem_Click(sender, e);
        }
        ...
        private void repositoryItemThem_Click(object sender, EventArgs e)
        {
            ...
            //int _IDDeBit = 0;
            string _madieuxe = gridView1.GetFocusedRowCellValue("MaDieuXe").ToString().Trim();
            frmFileDebitNcc frm = new frmFileDebitNcc(0, 0, 0, 0 ,0, _madieuxe);
            frm.ShowDialog();
            btnXem_Click(sender, e);
        }
        ...
        private void repositoryItemXoa_Click_1(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn có chắc chắn muốn xoá không (Y/N)", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ServiceReference1.WebService1SoapClient client = new ServiceReference1.WebService1SoapClient();
        ...
        private void btntaoDebit_Click(object sender, EventArgs e)
        {
            ...
            {
                if (bool.Parse(gridView1.GetRowCellValue(i, "Chon").ToString().Trim()))
                {
            ...
            else
            {
                using (var _ncc = new ncc())
                {
                    DataTable dt = _ncc.BangDieuXe_TaoDeBitHangLoat(str);
                    frmTaoDebit_File_NCC frm = new frmTaoDebit_File_NCC(dt);
                    frm.ShowDialog();
                }
        ...
        private void btnXuatHoaDon_Click(object sender, EventArgs e)
        {
            ...
            {
                if (bool.Parse(gridView2.GetRowCellValue(i, "ChonXuatHoaDon").ToString()))
                {
        ...
        {
            if (e.Column != null && e.Column.FieldName == "ChonXuatHoaDon")
            {
                e.Info.InnerElements.Clear(); // xóa nội dung mặc định
                e.Info.Appearance.DrawString(e.Cache, e.Info.Caption, e.Bounds);
        ...
        private void gridView2_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (hitInfo.InColumn && hitInfo.Column != null && hitInfo.Column.FieldName == "ChonXuatHoaDon")
            {
                if (headerCheckBoxRect.Contains(e.Location))
                {
                    // Toggle trạng thái checkbox header
                    ...
                    for (int i = 0; i < gridView2.RowCount; i++)
                    {
                        gridView2.SetRowCellValue(i, "ChonXuatHoaDon", headerCheckBoxState);
                    }

                    // Vẽ lại header
                    gridView2.InvalidateColumnHeader(gridView2.Columns["ChonXuatHoaDon"]);
                }
            }
        }
    }
