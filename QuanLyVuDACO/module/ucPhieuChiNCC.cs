...
{
    public partial class ucPhieuChiNCC : DevExpress.XtraEditors.XtraUserControl
    {
        public ucPhieuChiNCC()
        {
            InitializeComponent();
        }
        ...
        private void btnXem_Click(object sender, EventArgs e)
        {
            ...
            if (arr1.Length >= 3 && arr2.Length >= 3 && arr1[0].Trim() != "" && arr2[0].Trim() != "")
            {
                ...
                {
                    gridControl2.DataSource = _ncc.DanhSachPhieuChi_NCC_TheoNgay(Ngay1, Ngay2);
                }
                   // gridControl2.DataSource = client.DanhSachPhieuChi_NCC_TheoNgay(Ngay1, Ngay2);
            }
        }

        private void btnThemMoi_Click(object sender, EventArgs e)
        {
            Form frm = new Forms.frmPhieuChiNCC();
            frm.ShowDialog();
            btnXem_Click(sender, e);
        }
        ...
        private void repositoryItemSua_Click(object sender, EventArgs e)
        {
            ...
            if (_maChi != "001")
            {
                int _IDPhieuChi = int.Parse(gridView2.GetFocusedRowCellDisplayText("IDPhieuChiNCC"));
                Form frm = new Forms.frmPhieuChiNCC(_IDPhieuChi);
                frm.ShowDialog();
            }
        ...
        private void gridView2_CustomRowCellEditForEditing(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            try
            {
                if(e.RowHandle>=0)
                {
                    ...
                    gridControl1.DataSource = client.DanhSachPhieuChi_NCC_CT_TheoSoChungTu(_soChungTu);
                }    
            }
            catch (Exception ex)
            {
        ...
        {
            gridControl2.DataSource = client.DanhSachPhieuChi_NCC_TheoSoFile(txtSoFile.Text);
            gridControl1.DataSource = client.DanhSachPhieuChi_NCC_CT_TheoSoFile(txtSoFile.Text);
        }
