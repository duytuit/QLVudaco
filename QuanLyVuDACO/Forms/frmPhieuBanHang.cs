...
namespace Quản_lý_vudaco.Forms
{
    public partial class frmPhieuBanHang : DevExpress.XtraEditors.XtraForm
    {
        ...
        public frmPhieuBanHang(int IDPhieuChi)
        {
            InitializeComponent();
        ...
        private void frmPhieuChi_Load(object sender, EventArgs e)
        {
            ...
            cboDoiTuong_SelectedIndexChanged(sender, e);
            txtNguoiBan.Text = "";
            ...
            cboDanhSachThu.Properties.DataSource = tThu.ToList();
            repositoryItemXe.DataSource = client.ds_Xe();
            ...
            if (_IDPhieuChi!=0)
            {
                var table = client.DanhSachBanHang_TheoIDPhieuBan(_IDPhieuChi);
                foreach (var item in table)
                {
                    dtpNgayMua.Text = item.NgayBan.Value.ToString("dd/MM/yyyy");
                    txtDienGiai.Text = item.DienGiai;
                    cboDanhSachThu.EditValue = item.MaThu;
                 
                    cboKH.EditValue = item.MaNhaCC;
                    cboNhanVien.EditValue = item.MaNhanVien;
                    var table1 = client.DanhSachBanHang_CT_TheoIDPhieuBan(_IDPhieuChi);
                    foreach (var item1 in table1)
                    {

                        DataRow row = dtx.NewRow();
                        row["IDPhieuMuaCT"] = item1.IDPhieuBanCT;
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            try
            {
                 ...
                 if (gridView2.RowCount==0)
                    MessageBox.Show("Vui lòng nhập nội dung bán!");
                else if (cboDanhSachThu.EditValue == null)
                    MessageBox.Show("Vui lòng chọn danh mục thu!");
              
                else
                {
                    ServiceReference1.PhieuBan p = new ServiceReference1.PhieuBan();
                
                    p.DienGiai = txtDienGiai.Text;
                    p.MaThu = cboDanhSachThu.EditValue.ToString();
                    p.DoiTuong = cboDoiTuong.Text;
                    p.MaNhanVien = (cboNhanVien.EditValue == null) ? "" : cboNhanVien.EditValue.ToString();
                    p.NguoiBanHang = txtNguoiBan.Text;
                    p.MaNhaCC=(cboKH.EditValue == null) ? "" : cboKH.EditValue.ToString();
                    string[] arr = dtpNgayMua.Text.Trim().Split('/');
                    DateTime Ngay1 = new DateTime(int.Parse(arr[2]), int.Parse(arr[1]), int.Parse(arr[0]));
                    p.SoPhieu = client.Top1SoPhieu_BanHang(Ngay1);
                    p.NgayBan = Ngay1;
                    ...
                    List<ServiceReference1.PhieuBanCT> pCT = new List<ServiceReference1.PhieuBanCT>();
                    for (int i = 0; i < gridView2.RowCount-1; i++)
                    {

                        ServiceReference1.PhieuBanCT p1 = new ServiceReference1.PhieuBanCT();
                    ...
                    client.ThemPhieuBan(p, pCT.ToArray());
