...
{
    public partial class frmQuyNCC : DevExpress.XtraEditors.XtraForm
    {
        public frmQuyNCC()
        {
            InitializeComponent();
        ...
        private void btnXem_Click(object sender, EventArgs e)
        {
            try
            {
                if (cboLoaiQuy.EditValue != null)
                    frmCongNoNCC_CT._MaQuy = cboLoaiQuy.EditValue.ToString().Trim();
                else
                    frmCongNoNCC_CT._MaQuy = "";
                if (radioGroup1.SelectedIndex == 1)
                {
                    frmCongNoNCC_CT._SotK = lblSoTK.Text;
                    frmCongNoNCC_CT._ChuTK = lblChuTK.Text;
                    frmCongNoNCC_CT._TenNganHang = cboNganHang.Text;
                    frmCongNoNCC_CT._ChiNhanh = lblChiNhanh.Text;
                    frmCongNoNCC_CT._HinhThucTT = "CK";

                }
                else
                    frmCongNoNCC_CT._HinhThucTT = "TM";
                string[] arr1 = dtpNgay.Text.Split('/');
                DateTime Ngay1 = new DateTime(int.Parse(arr1[2]), int.Parse(arr1[1]), int.Parse(arr1[0]));
                frmCongNoNCC_CT._Ngay = Ngay1;
                frmCongNoNCC_CT._DienGiai = txtDienGiai.Text;
                this.Close();
            }
            catch (Exception)
            {
        ...
        private void frmQuyNCC_FormClosing(object sender, FormClosingEventArgs e)
        {
