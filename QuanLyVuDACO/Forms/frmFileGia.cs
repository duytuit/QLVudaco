...
{
    public partial class frmFileGia : DevExpress.XtraEditors.XtraForm
    {
        public frmFileGia()
        {
            InitializeComponent();
        }
        public frmFileGia(int IDLoHang)
        {
            InitializeComponent();
            _IDLoHang = IDLoHang;
        }
        public frmFileGia(int IDLoHang, int sua)
        {
            InitializeComponent();
            _IDLoHang = IDLoHang;
            _sua = sua;
        }
        public frmFileGia(int IDLoHang,string SoFile, int sua)
        {
            InitializeComponent();
            _IDLoHang = IDLoHang;
            _sua = sua;
        ...
        public frmFileGia(int IDLoHang, string SoFile,int _IDGia, int sua)
        {
            InitializeComponent();
            _IDLoHang = IDLoHang;
            _sua = sua;
        ...
        public frmFileGia(int IDLoHang, string SoFile, int _IDGia,int IDCP, int sua)
        {
            InitializeComponent();
            _IDLoHang = IDLoHang;
            _sua = sua;
        ...
        private void frmBangKeChiPhi_Load(object sender, EventArgs e)
        {
          ...
          var table1=  client.ThongTinFile_TheoIDLoHang(_IDLoHang);
            foreach (var item in table1)
            {
                dtpNgay.Text = item.ThoiGianThucHien.Value.ToString("dd/MM/yyyy");
            }
            cboNCC.Properties.DataSource= client.ds_NCC();
            repositoryItemNhaCC.DataSource = client.ds_NCC();
            var dtCP= client.BangLietKeCP_TheoIDCP(_idcp);
            foreach (var item in dtCP)
            {
                txtLyDoDuyet.Text = item.LyDoDuyet;
            }
            ...
            if(dt.Rows.Count>0)
            {
                ...
                // DataTable dtCT = client.DsChiPhiHQ_TheoSoFile2(lblSoFile.Text, _IDLoHang,_idcp);
                using (var _ncc = new ncc())
                {
                    DataTable dtCT = _ncc.DsChiPhiHQ_TheoSoFile2(lblSoFile.Text, _IDLoHang,_idcp);
                    dtCT.Columns.Add("LinhVucKhac", typeof(bool));
                    for (int i = 0; i < dtCT.Rows.Count; i++)
                    {
                        dtCT.Rows[i]["LinhVucKhac"] = false;
                        string dfgdfg = dtCT.Rows[i]["MaNhaCungCap"].ToString();
                        if (dtCT.Rows[i]["MaNhaCungCap"].ToString() != "")
                        {
                            cboNCC.EditValue = dtCT.Rows[i]["MaNhaCungCap"].ToString();
                          //  repositoryItemNhaCC.DisplayMember = dtCT.Rows[i]["TenNhaCungCap"].ToString();  // Cột hiển thị
                          //  repositoryItemNhaCC.ValueMember = dtCT.Rows[i]["MaNhaCungCap"].ToString();
                          //  gridView1.SetRowCellValue(0, dtCT.Rows[i]["TenNhaCungCap"].ToString(), dtCT.Rows[i]["MaNhaCungCap"].ToString());
                        }
                    }
            ...
            if(_sua==1)
            {
               var t2= client.FileGia_TheoIDGia(IDGia);
                ...
                dt1.Columns.Add("LinhVucKhac", typeof(bool));
                foreach (var item in t)
                {
                    ...
                    if (item.LinhVucKhac != null)
                        row["LinhVucKhac"] = item.LinhVucKhac.Value;
                    else
                        row["LinhVucKhac"] = false;
        ...
        private void btnLuu_Click_1(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                bool isCheckNCC_Ngoai = client.KiemTraNCC_Ngoai_TheoIDLoHang(lblSoFile.Text);
                if (isCheckNCC_Ngoai)
                {
                    ...
                    for (int i = 0; i < gridView1.RowCount; i++)
                    {
                        if (!gridView1.IsNewItemRow(i))
                        {
                            ...
                            p2.LinhVucKhac = bool.Parse(gridView1.GetRowCellValue(i, "LinhVucKhac").ToString().Trim());
                            string sdf = (gridView1.GetRowCellValue(i, "MaNhaCungCap") != null) ? gridView1.GetRowCellValue(i, "MaNhaCungCap").ToString().Trim() : "";
                            p2.MaNhaCungCap = sdf;
                            if (p2.TenDichVu != "")
