...
{
    public partial class frmFileDebitNcc : DevExpress.XtraEditors.XtraForm
    {
        public int _Id_debit { get; private set; } = 0;
        public frmFileDebitNcc()
        {
            InitializeComponent();
        }
        public frmFileDebitNcc(int IDGia)
        {
            InitializeComponent();
            _IDGia = IDGia;
        }
        public frmFileDebitNcc(int IDGia,int sua)
        {
            InitializeComponent();
            _IDGia = IDGia;
            _sua = sua;
        }
        public frmFileDebitNcc(int IDLoHang,int IDGia, int sua)
        {
            InitializeComponent();
            _IDLoHang = IDLoHang;
            _IDGia = IDGia;
        ...
        public frmFileDebitNcc(int IDLoHang, int IDGia, int sua, int ncc, int kh ,string madieuxe = null)
        {
            InitializeComponent();
            _IDLoHang = IDLoHang;
            _IDGia = IDGia;
        ...
        public frmFileDebitNcc(int IDDeBit,int IDLoHang, int IDGia, int sua)
        {
            InitializeComponent();
            _IDDeBit = IDDeBit;
            _IDLoHang = IDLoHang;
                        ...
                        else
                        {
                            ...
                            for (int i = 0; i < gridView1.RowCount; i++)
                            {
                                if (!gridView1.IsNewItemRow(i))
                                {
                                    ...
                                    p2.GhiChu = gridView1.GetRowCellDisplayText(i, "GhiChu").Trim();
                                    p2.LaPhiChiHo = bool.Parse(gridView1.GetRowCellValue(i, "LaPhiChiHo").ToString().Trim());
                                    p2.IDKey = int.Parse(gridView1.GetRowCellDisplayText(i, "IDGiaCT").ToString().Trim());
                                    p2.KeyName = "filegiachitiet";
                                    if (p2.TenDichVu != "")
                                    {
                                        int a = _appDB.UpsertFromObject("FileDebitNccChiTiet", p2, "IDDeBitCT", true);
                                    }
