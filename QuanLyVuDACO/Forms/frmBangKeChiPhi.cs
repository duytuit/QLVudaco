...
{
    public partial class frmBangKeChiPhi : DevExpress.XtraEditors.XtraForm
    {
        public frmBangKeChiPhi()
        {
            InitializeComponent();
        }
        public frmBangKeChiPhi(int IDLoHang)
        {
            InitializeComponent();
            _IDLoHang = IDLoHang;
        }
        public frmBangKeChiPhi(int IDCP,int sua, int k)
        {
            InitializeComponent();
            _IDCP = IDCP;
        }
        public frmBangKeChiPhi(int IDCP,int sua)
        {
            InitializeComponent();
            _IDCP = IDCP;
            _sua = sua;
        }
        public frmBangKeChiPhi(int IDCP, int sua,int temp,int ThemMoi)
        {
            InitializeComponent();
            _IDCP = IDCP;
            _sua = sua;
        ...
        int _IDCP = 0, _sua = 0, _IDLoHang = 0, _ThemMoi = 0;
        ...
        private void frmBangKeChiPhi_Load(object sender, EventArgs e)
        {
            ...
            if (_IDLoHang!=0)//IDCP
            {
                ...
                using ( var _lietkechiphi = new lietkechiphi() )
                {
                    DataTable table = _lietkechiphi.DsThongTinFile_Full_TheoIDLoHang(_IDLoHang);
                   //DataTable table = client.DsThongTinFile_Full_TheoIDLoHang(_IDLoHang);
                    if (table.Rows.Count > 0)
                    {
                        foreach (DataRow item in table.Rows)
                        {
                            ...
                            dtpNgayTao.Text = Convert.ToDateTime(item["ThoiGianThucHien"]).ToString("dd/MM/yyyy");
                            lblLoaiToKhai.Text = item["LoaiToKhai"].ToString();
            ...
            if(_sua==1)
            {
                ...
                for (int i = 0; i < dt1.Rows.Count; i++)
                {
                    ...
                    row["LinkTaiHoaDon"] = dt1.Rows[i]["LinkTaiHoaDon"].ToString();
                    row["MaTraCuu"] = dt1.Rows[i]["MaTraCuu"].ToString();
        ...
        private void simpleButton2_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                gridView2.SetFocusedRowCellValue("LinkTaiHoaDon", txtLink.Text);
                gridView2.SetFocusedRowCellValue("MaTraCuu", txtMa.Text);
                gridView2.FocusedRowHandle = 0;
                TongTienLoHang();
            }
        }
        ...
        private void ThemBangKe(int ThemMoi)
        {
            ...
            if (gridView1.RowCount > 0)
                ...
                for (int i = 0; i < gridView2.RowCount; i++)
                {
                    ...
                    table2.LinkTaiHoaDon= gridView2.GetRowCellValue(i, "LinkTaiHoaDon").ToString();
                    table2.MaTraCuu = gridView2.GetRowCellValue(i, "MaTraCuu").ToString();
                   table2.IDCP = id;
                      client.BangKeCPCT_Them(table2);
                    }
                }
            if (ThemMoi == 1)
            {
                ...
                p.IDCP = id;
                p.ThoiGianTao = Convert.ToDateTime(table1.NgayTaoBangKe);
