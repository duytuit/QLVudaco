...
{
    public partial class frmCongNoNCC_CT_V3 : DevExpress.XtraEditors.XtraForm
    {
        public frmCongNoNCC_CT_V3()
        {
            InitializeComponent();
            // colSTT.UnboundType = DevExpress.Data.UnboundColumnType.Integer;
            gridView1.Columns["STT"].UnboundType = DevExpress.Data.UnboundColumnType.Integer;
        ...
        private void GridView1_CustomUnboundColumnData(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            ...
            {
                double ChiNangHa = Convert.ToDouble(gridView1.GetListSourceRowCellValue(e.ListSourceRowIndex, "ChiNangHa"));
                double ThanhTien = Convert.ToDouble(gridView1.GetListSourceRowCellValue(e.ListSourceRowIndex, "ThanhTien"));
                double UngTruocNCC = Convert.ToDouble(gridView1.GetListSourceRowCellValue(e.ListSourceRowIndex, "UngTruocNCC"));
                e.Value = (ChiNangHa + ThanhTien) - UngTruocNCC;
            }

        }
        public frmCongNoNCC_CT_V3(DateTime TuNgay,DateTime DenNgay,string MaKH,string TenKH = null)
        {
            InitializeComponent();
            _TuNgay = TuNgay;
            _DenNgay = DenNgay;
                ...
                using (var _ncc = new ncc())
                {
                    ...
                    var ncc_ct = _ncc.CongNoChiTietNcc(_TuNgay, _DenNgay, _MaKH)
                                    .Select(x => new
                                    {
                                        ...
                                        BienSoXe = x.BienSoXe,
                                        SoCont = x.SoCont,
                                        SoTien = (x.Type == 1 || x.Type == 4) ? 0 : x.SoTien,
                                        TienVAT = (x.Type == 1 || x.Type == 4) ? 0 : x.VAT,
                                        ThanhTien = (x.Type == 1 || x.Type == 4) ? 0 : x.ThanhTien,
                                        ChiNangHa = (x.Type == 1) ? x.ThanhTien : 0,
                                        UngTruocNCC = (x.Type == 4) ? x.ThanhTien : 0,
                                        Type = x.Type,
                                        Chon = x.Chon,
                                    })
                                    .OrderBy(x => x.Ngay)
                    ...
                    rpt.DataSource = ToDataTable(ncc_ct);
        ...
        private void btnPhieuThu_Click(object sender, EventArgs e)
        {
            ...
            MessageBox.Show("Dịch vụ đã được thanh toán");
        }
            ...
            try
            {
                if (e.RowHandle >= 0 && e.Column.FieldName == "Chon")
                {
                    ...
                    if (isCheck)
                    {
                        ...
                        double chiNangHa = Convert.ToDouble(gridView1.GetRowCellValue(e.RowHandle, "ChiNangHa") ?? 0);
                        double thanhTien = Convert.ToDouble(gridView1.GetRowCellValue(e.RowHandle, "ThanhTien") ?? 0);
                        double ungTruocNCC = Convert.ToDouble(gridView1.GetRowCellValue(e.RowHandle, "UngTruocNCC") ?? 0);

                        double conLai = (chiNangHa + thanhTien) - ungTruocNCC;
